...
function pinfo_char.is_character_veteran(character_info)
    if character_info.veteran ~= nil then
        ...
        local veteranness = IsUnitVeteran(pinfo_char.PLAYER_UNIT_TAG)
...
local function _get_character_level_xp(character_info)
    if pinfo_char.is_character_veteran(character_info) == false then
        return GetUnitXP(pinfo_char.PLAYER_UNIT_TAG)
    else
        return GetUnitVeteranPoints(pinfo_char.PLAYER_UNIT_TAG)
    end
end
...
local function _get_character_level(character_info)
    if pinfo_char.is_character_veteran(character_info) == false then
        return GetUnitLevel(pinfo_char.PLAYER_UNIT_TAG)
    else
        return GetUnitVeteranRank(pinfo_char.PLAYER_UNIT_TAG)
    end
end
...
function pinfo_char.get_character_gender(character_info)
    if character_info.gender ~= nil then
        ...
        local gender = GetUnitGender(pinfo_char.PLAYER_UNIT_TAG)
...
function pinfo_char.get_character_ava_rank(character_info)
    if character_info.ava_rank ~= nil then
        ...
        local ava_rank, ava_sub_rank = GetUnitAvARank(pinfo_char.PLAYER_UNIT_TAG)
...
function pinfo_char.get_character_class(character_info)
    if character_info.class ~= nil then
        ...
        local class = GetUnitClass(pinfo_char.PLAYER_UNIT_TAG)
...
local function _get_character_level_xp_max(character_info)
    if pinfo_char.is_character_veteran(character_info) == false then
        return GetUnitXPMax(pinfo_char.PLAYER_UNIT_TAG)
    else
        return GetUnitVeteranPointsMax(pinfo_char.PLAYER_UNIT_TAG)
    end
end
...
function pinfo_char.get_character_rank_points(character_info)
    if character_info.rank_points ~= nil then
        return character_info.rank_points
    else
        local rank_points = GetUnitAvARankPoints(pinfo_char.PLAYER_UNIT_TAG)
        character_info.rank_points = rank_points
        return rank_points
    end
end

function pinfo_char.get_character_rank_points_max(character_info)
    if character_info.rank_points_max ~= nil then
        return character_info.rank_points_max
    else
        local rank_points = pinfo_char.get_character_rank_points(character_info)
...
function pinfo_char.get_character_rank_points_percent(character_info)
    local rank_points_max = pinfo_char.get_character_rank_points_max(character_info)
    local rank_points = pinfo_char.get_character_rank_points(character_info)
